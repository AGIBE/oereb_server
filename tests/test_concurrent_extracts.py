...
def test_concurrent_requests(running_server_instance, egrids_for_concurrent_error):
    ...
    plr_count_slow = len(
        res_slow.json()["GetExtractByIdResponse"]["extract"]["RealEstate"][
            "RestrictionOnLandownership"
        ]
    )
    plr_count_fast = len(
        res_fast.json()["GetExtractByIdResponse"]["extract"]["RealEstate"][
            "RestrictionOnLandownership"
        ]
    )

    sequential_result = {slow_egrid: plr_count_slow, fast_egrid: plr_count_fast}

    concurrent_result = {}

    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        for result in executor.map(
            count_plr_in_json_extract, [slow_extract_url, fast_extract_url]
        ):
