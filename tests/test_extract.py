...
def test_largest_area_extract_json(running_server_instance, largest_area_parcel):
    extract_url = (
        running_server_instance + "/extract/json/?egrid=" + largest_area_parcel
    )
...
@pytest.mark.xfail(
    reason="Seit den Gewässerschutzbereichen gibt es keine Parzellen ohne ÖREBs mehr."
)
...
def test_complex_parcel_json(running_server_instance, complex_area_parcel):
    extract_url = (
        running_server_instance + "/extract/json/?egrid=" + complex_area_parcel
    )
    res = requests.get(extract_url)
    assert res.status_code == 200
    assert "GetExtractByIdResponse" in res.json().keys()
...
def test_complex_extract_json(running_server_instance, complex_extract_parcel):
    extract_url = (
        running_server_instance + "/extract/json/?egrid=" + complex_extract_parcel
    )
    res = requests.get(extract_url)
    assert res.status_code == 200
    assert "GetExtractByIdResponse" in res.json().keys()
...
def test_complex_extract_pdf(running_server_instance, complex_extract_parcel):
    extract_url = (
        running_server_instance + "/extract/pdf/?egrid=" + complex_extract_parcel
    )
    res = requests.get(extract_url)
    assert res.status_code == 200


def test_large_geometry_error_extract_pdf(
    running_server_instance, egrid_large_geometries_error
):
    ...
    extract_url = (
        running_server_instance + "/extract/pdf/?egrid=" + egrid_large_geometries_error
    )
...
def test_extract_building_right(running_server_instance, parcel_building_right):
    extract_url = (
        running_server_instance + "/extract/json/?egrid=" + parcel_building_right
    )
    res = requests.get(extract_url)
    json = res.json()
    real_estate_type = json["GetExtractByIdResponse"]["extract"]["RealEstate"]["Type"][
        "Text"
    ][0]["Text"]
    assert res.status_code == 200
...
def test_extract_sources_right(running_server_instance, parcel_source_right):
    extract_url = (
        running_server_instance + "/extract/json/?egrid=" + parcel_source_right
    )
    res = requests.get(extract_url)
    json = res.json()
    real_estate_type = json["GetExtractByIdResponse"]["extract"]["RealEstate"]["Type"][
...
def test_extract_concession_right(running_server_instance, parcel_concession_right):
    extract_url = (
        running_server_instance + "/extract/json/?egrid=" + parcel_concession_right
    )
    res = requests.get(extract_url)
    json = res.json()
    real_estate_type = json["GetExtractByIdResponse"]["extract"]["RealEstate"]["Type"][
...
def test_extract_xml_valid_schema(
    running_server_instance, egrid_with_some_plr, schema_for_validation
):
...
def test_extract_gz_new(running_server_instance, nbident_number_gz_new):
    ...
    extract_url = (
        f"{running_server_instance}/extract/json/?identdn={nbident}&number={number}"
    )
    res = requests.get(extract_url)
    json = res.json()
