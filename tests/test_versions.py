...
def test_versions_json_valid(running_server_instance):
    json_schema = requests.get(
        "http://schemas.geo.admin.ch/V_D/OeREB/2.0/versioning.json"
    ).json()
    url = running_server_instance + "/versions/json"
    res = requests.get(url)
    json_res = res.json()
...
def test_versions_xml_number_of_versions(running_server_instance):
    ...
    number_of_versions_xml = len(
        xml_root.findall(
            "{http://schemas.geo.admin.ch/V_D/OeREB/1.0/Versioning}supportedVersion"
        )
    )
...
def test_versions_xml_version_number(running_server_instance):
    ...
    res = requests.get(url)
    xml_root = ET.fromstring(res.text)
    spVer = xml_root.findall(
        "{http://schemas.geo.admin.ch/V_D/OeREB/1.0/Versioning}supportedVersion"
    )[0]
    version = spVer.findall(
        "{http://schemas.geo.admin.ch/V_D/OeREB/1.0/Versioning}version"
    )[0]
...
def test_versions_xml_valid(running_server_instance):
    ...
    schema = xmlschema.XMLSchema(
        "http://schemas.geo.admin.ch/V_D/OeREB/2.0/Versioning.xsd"
    )
