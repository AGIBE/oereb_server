...
async def fast_request_group(session, url, i, expected_length):
    ...
    for j, resp in enumerate(responses):
        ...
        assert actual == expected_length, f"fast_request Task-{i} Run-{j}: Länge {actual} != erwartet {expected_length} {legend_texts}"
...
async def slow_request(session, url, expected_length):
    ...
    assert actual == expected_length, f"slow_request: Länge {actual} != erwartet {expected_length}"
    return response
