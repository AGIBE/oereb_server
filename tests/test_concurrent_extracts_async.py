...
async def fast_request_group(session, url, i, expected_length):
    tasks = [make_request(session, url, i, j) for j in range(6)]
    ...
    for j, resp in enumerate(responses):
        ...
        legend_texts = []
        for plr in resp.json()["GetExtractByIdResponse"]["extract"]["RealEstate"][
            "RestrictionOnLandownership"
        ]:
            legend_texts.append(plr["LegendText"][0]["Text"])
        assert actual == expected_length, (
            f"fast_request: Länge {actual} != erwartet {expected_length} {legend_texts}"
        )
...
async def slow_request(session, url, expected_length):
    ...
    assert actual == expected_length, (
        f"slow_request: Länge {actual} != erwartet {expected_length} {response.request.url}"
    )
    return response
...
@pytest.mark.asyncio
async def test_parallel_requests_with_reference_check(
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        ...
        slow_task = asyncio.create_task(
            slow_request(client, slow_extract_url, slow_ref_len)
        )

        fast_tasks = [
            asyncio.create_task(
                fast_request_group(client, fast_extract_url, i, fast_ref_len)
            )
            for i in range(4)
        ]
