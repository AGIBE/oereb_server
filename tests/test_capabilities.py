...
def test_capabilities_json_municipalities(running_server_instance, number_of_municipalities):
    url = running_server_instance + "/capabilities/json"
    res = requests.get(url)
    res_json = res.json()
...
def test_capabilities_xml_municipality(running_server_instance, number_of_municipalities):
    url = running_server_instance + "/capabilities/xml"
    res = requests.get(url)
    xml_root = ET.fromstring(res.text)
...
def test_capabilities_xml_flavour(running_server_instance):
    ...
    res = requests.get(url)
    xml_root = ET.fromstring(res.text)
    flavour = xml_root.findall("{http://schemas.geo.admin.ch/V_D/OeREB/2.0/Extract}flavour")
    assert len(flavour) == 1
...
def test_capabilities_language_flavour(running_server_instance):
    ...
    res = requests.get(url)
    xml_root = ET.fromstring(res.text)
    languages = xml_root.findall("{http://schemas.geo.admin.ch/V_D/OeREB/2.0/Extract}language")
